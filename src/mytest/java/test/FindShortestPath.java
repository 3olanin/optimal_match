...
public class FindShortestPath {
	private static File basePath = new File("D:/");
    private static File dbPath = new File(basePath, "Neo4jDB_2_2");
...
//	    	Node nid2=osmDs.getNodeFromOSMId("1422597068");
	    	Node nid1=db.getNodeById(52574);
	    	Node nid2=db.getNodeById(39259);
	    	...
	    	int minute_id = 0;
	    	while(n<10000) {
	    		WeightedPath p1=osmDs.getdijkstraLengthShortestPath(nid1, nid2, minute_id);
	    		if(n==0){
	    			Iterable<Relationship> rr = p1.relationships();
	    			//Iterator<Relationship> itr= p1.relationships().iterator();
	    			Iterator<Relationship> itr=rr.iterator();
	    			System.out.println("weight = " + p1.weight());
	    			System.out.println("distance = " + PathManager.getWeightedPathDistance(p1));
	    			while(itr.hasNext())
	    			{
	    				...
	    				System.out.println(r.getProperty("length"));
	    			}
	    			System.out.println(p1);
	    			
	    			LinkedList<Relationship> list = iterable2LinkedListPath(rr);
	    			while(!list.isEmpty())
	    			{
	    				Relationship r = list.poll();
	    				//if(r.isType(OSMRelation.NEXT))
	    				System.out.println(r.getProperty("length"));
	    			}
	    		}
	    			//System.out.println("length:"+p1.length()+",weight:"+p1.weight());
	    		n++;
	    		break;
	    	}
